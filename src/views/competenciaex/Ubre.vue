...
export default {
  ...
  data() {
    return {
      ...
      title: "Competencias de Especiales - Mejor Ubre",
      page: 1,
      pageCount: 0,
      search: "",
      tipo: "Categoria",
  ...
  methods: {
    getCompe() {
      axios
        .get(`competitionsEx/listCompUbre/`)
        .then((res) => {
    ...
    editItem(item) {
      ...
      this.title_card = "MEJOR UBRE - CAPRINOS";
      this.item = item;
      this.open = true;
    },
    closed() {
      ...
      this.$swal
        ...
        .then((result) => {
          if (result.isConfirmed) {
            axios
              .post("/competitionsEx/saveUbre", data)
              .then((res) => {
                this.$toast.open({
